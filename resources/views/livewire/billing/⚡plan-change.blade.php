                ...
                <dl class="grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-4">
                    @foreach (['newPriceNet' => __('billing::portal.net'), 'vatAmount' => __('billing::portal.vat'), 'couponDiscountNet' => __('billing::portal.discount'), 'grossTotal' => __('billing::portal.gross')] as $key => $label)
                        @if (isset($preview[$key]) && $preview[$key] != 0)
                            <div>
                                <flux:subheading size="sm" class="text-zinc-400 dark:text-zinc-500">{{ $label }}</flux:subheading>
                                <flux:text class="mt-1 font-semibold tabular-nums {{ $key === 'grossTotal' ? 'text-lg' : '' }}">
