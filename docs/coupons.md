...
| **`trial_extension`** | Extends `trial_ends_at` by `trial_extension_days`. For Mollie-source subscriptions also PATCHes the Mollie `startDate` to the new trial end so the next charge follows the extended trial. | Checkout (with trial-gate), Dashboard (with trial-gate) | Immediately on redemption — `extendBillingTrialUntil()` shifts the trial end; for Mollie subscriptions `MollieSubscriptionPatcher::setNextChargeDate()` aligns the Mollie schedule. | Never. |
