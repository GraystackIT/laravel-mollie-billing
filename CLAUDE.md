...
6. **Livewire 4 + Flux Pro UI** — `livewire/flux-pro` is a **hard dependency**. All portal/admin views use `<flux:…>` components directly — no fallback wrappers, no `FluxPro::isInstalled()` checks. Consuming apps ship their own checkout/plan-selection flow; the package provides the post-signup portal only (dashboard, plan change, invoices, return). The app's checkout route name is configurable via `mollie-billing.checkout_route` — `PromotionController`, `RequireActiveSubscription` middleware, and `TrialExpiredNotification` all redirect there.
